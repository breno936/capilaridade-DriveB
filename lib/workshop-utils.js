import { detectWorkshopBrand } from './workshop-brands.js';
import { resolveCategoria } from './workshop-categories.js';
...
export const mapThemeOptions = [
  { value: 'light', label: 'Relevo' },
  { value: 'voyager', label: 'Ruas' },
  { value: 'satellite', label: 'Satelite' },
  { value: 'dark', label: 'Escuro' },
];

export const cityResolutionOptions = [
  { value: 'all', label: 'Todas' },
  { value: 'resolved', label: 'Com cidade' },
  { value: 'unresolved', label: 'Sem cidade' },
];
...
export const strategicViewOptions = [
  { value: 'city', label: 'Cidade' },
  { value: 'state', label: 'Estado' },
  { value: 'region_service', label: 'Regiao servico' },
  { value: 'region_part', label: 'Regiao pecas' },
];

export const solutionOptions = [
  { value: 'all', label: 'Todos' },
  { value: '1', label: 'DriveB' },
  { value: '2', label: 'Outros' },
];

export function solutionLabel(franchiseId) {
  return solutionOptions.find((option) => option.value === String(franchiseId))?.label || 'Nao identificado';
}
...
const consultantRules = [
  ...
  {
    label: 'Caue Conen',
    macroRegions: ['Sudeste', 'Centro-Oeste', 'Nordeste', 'Norte'],
  },
];
...
const tireNameRegex = /(pneu|pneus|borrachar|campneus|recap|tyre|pneustil|pneumac)/;
const glassNameRegex = /(vidro|vidros|parabrisa|parabrisas|autoglass|glass|vigia|poliglass)/;
...
export function buildLookupKey(item) {
  if (item.lookupKey) return String(item.lookupKey);
  if (item.addressId) return `address:${item.addressId}`;
  if (Number.isFinite(item.lat) && Number.isFinite(item.lng)) {
    return `coord:${Number(item.lat).toFixed(5)},${Number(item.lng).toFixed(5)}`;
  }
  return '';
}

export function buildCityCacheMap(entries = []) {
  return entries.reduce((map, entry) => {
    if (!entry?.lookupKey) return map;
    map[entry.lookupKey] = entry;
    return map;
  }, {});
}

export function buildWorkshopCategoriesMap(entries = []) {
  return entries.reduce((map, entry) => {
    if (!Number.isFinite(entry?.id)) return map;
    map[entry.id] = entry;
    return map;
  }, {});
}
...
function resolveConsultant(stateCode) {
  if (!stateCode) return '';
  const macroRegion = macroRegionByState[stateCode] || '';
  const match = consultantRules.find((rule) => {
    if (rule.states?.includes(stateCode)) return true;
    if (rule.macroRegions?.includes(macroRegion)) return true;
    return false;
  });
  return match?.label || '';
}
...
export function formatStateLabel(code) {
  if (!code) return 'Nao identificado';
...
export function enrichWorkshop(item, cityCacheMap = {}, categoriesMap = {}) {
  const lookupKey = buildLookupKey(item);
  const cacheEntry = cityCacheMap[lookupKey] || null;
  const cityName = item.cityName || cacheEntry?.cityName || '';
  const cityStateCode = item.cityStateCode || cacheEntry?.cityStateCode || '';
  const stateCode = cityStateCode || deriveStateFromCoordinates(item.lat, item.lng) || deriveStateFromPhone(item);
  const normalizedCityStateCode = cityStateCode || stateCode;
  const cityKey = item.cityKey || cacheEntry?.cityKey || (cityName && normalizedCityStateCode ? `${normalizeText(cityName)}::${normalizedCityStateCode}` : '');
  const responsibleLabel = item.ownerName || item.ownerEmail || '';
  const consultantLabel = resolveConsultant(stateCode);
  const isInBrazil = typeof cacheEntry?.isBrazil === 'boolean' ? cacheEntry.isBrazil : Boolean(item.isInBrazil);

  const categoryEntry = categoriesMap[item.id] || null;
  const categoria = categoryEntry?.categoria || '';
  const negotiationTables = categoryEntry?.tabelas || [];
  const resolvedCategoria = categoria ? resolveCategoria(categoria) : null;

  const serviceType = resolvedCategoria ? resolvedCategoria.serviceType : deriveCoverageType(item);
  // coverageType feeds the 3-pillar gap math (oficina/vidros/pneus) across the app; finer-grained
  // types (funilaria, concessionaria) still count as a general oficina there so they don't vanish
  // from that calculation. The full detail lives in serviceType below.
  const coverageType = (serviceType === 'funilaria' || serviceType === 'concessionaria') ? 'oficina' : serviceType;
  const brand = resolvedCategoria
    ? { slug: resolvedCategoria.brandSlug, label: resolvedCategoria.brandLabel, hasIcon: resolvedCategoria.hasIcon }
    : detectWorkshopBrand(item);

  return {
    ...item,
    lookupKey,
    addressId: item.addressId || cacheEntry?.addressId || '',
    isInBrazil,
    stateCode,
    stateName: stateNameByCode[stateCode] || '',
    cityName,
    cityStateCode: normalizedCityStateCode,
    cityKey,
    cityDisplayName: cityName ? `${cityName}${normalizedCityStateCode ? ` · ${normalizedCityStateCode}` : ''}` : '',
    citySource: item.citySource || cacheEntry?.source || '',
    hasResolvedCity: Boolean(cityName && cityKey),
    franchiseId: String(item.franchiseId || ''),
    responsibleLabel,
    consultantLabel,
    macroRegion: macroRegionByState[stateCode] || '',
    regionServiceLabel: item.regionServiceId ? `Servico ${item.regionServiceId}` : '',
    regionPartLabel: item.regionPartId ? `Pecas ${item.regionPartId}` : '',
    coverageType,
    coverageLabel: coverageTypeLabel(coverageType),
    serviceType,
    serviceTypeLabel: coverageTypeLabel(serviceType),
    categoria,
    negotiationTables,
    brandSlug: brand?.slug || '',
    brandLabel: brand?.label || '',
    brandHasIcon: brand?.hasIcon !== false,
    isOperational: Boolean(item.isActive && !item.isBlocked),
  };
}

export function enrichWorkshops(items, cityCacheEntries = [], categoriesEntries = []) {
  const cityCacheMap = Array.isArray(cityCacheEntries) ? buildCityCacheMap(cityCacheEntries) : cityCacheEntries;
  const categoriesMap = Array.isArray(categoriesEntries) ? buildWorkshopCategoriesMap(categoriesEntries) : categoriesEntries;
  return items.map((item) => enrichWorkshop(item, cityCacheMap, categoriesMap));
}
...
export function countBy(items, key) {
  return items.reduce((accumulator, item) => {
    const bucket = item[key] || 'Nao informado';
...
export function filterWorkshops(items, filters) {
  ...
  return items.filter((item) => {
    ...
    if (filters.consultant !== 'all' && item.consultantLabel !== filters.consultant) return false;
    if (filters.responsible !== 'all' && item.responsibleLabel !== filters.responsible) return false;
    if (filters.concept !== 'all' && item.concept !== filters.concept) return false;
    if (filters.franchiseId !== 'all' && String(item.franchiseId) !== filters.franchiseId) return false;
    if (filters.franchiseId === '1' && !item.isInBrazil) return false;
    ...
    if (filters.brand !== 'all' && item.brandSlug !== filters.brand) return false;
    if (filters.cityResolution === 'resolved' && !item.hasResolvedCity) return false;
    if (filters.cityResolution === 'unresolved' && item.hasResolvedCity) return false;
...
function getGroupingDescriptor(item, dimension) {
  ...
  if (dimension === 'region_service') {
    if (!item.regionServiceId) return null;
    return {
      key: `service:${item.regionServiceId}`,
      name: item.regionServiceLabel || `Servico ${item.regionServiceId}`,
    };
  }

  if (dimension === 'region_part') {
    if (!item.regionPartId) return null;
    return {
      key: `part:${item.regionPartId}`,
      name: item.regionPartLabel || `Pecas ${item.regionPartId}`,
    };
  }
...
export function buildCoverageReports(items, dimension = 'state') {
  ...
  return Object.values(groups).map((group) => {
    ...
    const minimumGapPercent = (minimumGap / 3) * 100;
    const idealGapPercent = (idealGap / 6) * 100;
    ...
    const stateHints = [...new Set(group.items.map((item) => item.stateCode).filter(Boolean))]
      .slice(0, 3)
      .map((code) => formatStateLabel(code));
    const cityHints = [...new Set(group.items.map((item) => item.cityDisplayName).filter(Boolean))].slice(0, 2);
    const locationHint = dimension === 'city'
      ? ''
      : cityHints.length
        ? cityHints.join(' | ')
        : stateHints.join(' | ');
    const consultantHints = [...new Set(group.items.map((item) => item.consultantLabel).filter(Boolean))];
    const consultantLabel = consultantHints.length === 1 ? consultantHints[0] : consultantHints.length > 1 ? 'Multiplas zonas' : '';
...
export function buildStrategicSummary(items, dimension = 'state') {
  ...
  const topPriorityStates = reports.filter((report) => report.status !== 'ideal');
  ...
  criticalStates.slice(0, 3).forEach((report) => {
    ...
    recommendations.push({
      ...
      body: `Hoje faltam pilares minimos de cobertura em ${missing}. Para sair do critico, ${dimension === 'city' ? 'a cidade' : 'a localidade'} precisa de ${report.minimumDeficitText}.`,
    });
  });
  ...
  attentionStates
    ...
    .forEach((report) => {
      recommendations.push({
        tone: 'attention',
        title: `${report.groupName} e oportunidade de adensamento`,
        body: `A base ja existe, mas ainda faltam ${report.idealDeficitText} para atingir a meta ideal de 2 oficinas, 2 pontos de vidros e 2 de pneus.`,
      });
    });
  ...
  if (!recommendations.length) {
    recommendations.push({
      ...
      body: 'As localidades visiveis ja cumprem a meta minima. O proximo passo e reforcar os pontos com maior volume para ganhar redundancia operacional.',
    });
  }

  const headline = criticalStates.length
    ? `${formatNumber(criticalStates.length)} ${dimension === 'city' ? 'cidades' : 'localidades'} estao criticas no recorte atual`
    : attentionStates.length
      ? `${formatNumber(attentionStates.length)} ${dimension === 'city' ? 'cidades' : 'localidades'} ainda precisam de reforco`
      : 'Cobertura minima atendida no recorte atual';

  const narrative = criticalStates.length
    ? `A leitura estrategica considera apenas operacoes ativas e nao bloqueadas. Os itens criticos sao os que ainda nao possuem pelo menos 1 oficina, 1 ponto de vidros e 1 de pneus.`
    : 'A meta minima esta coberta, entao o foco passa a ser adensar para o padrao ideal de 2 operacoes por pilar.';
...
export function downloadCsv(items) {
  const header = [
    ...
    'franchise_id', 'consultant', 'state_code', 'state_name', 'city_name', 'city_state_code', 'coverage_type', 'is_active', 'is_blocked', 'is_offline', 'checkout_type',
...
export function downloadCommercialCsv(items) {
  const header = ['oficina', 'cidade', 'estado', 'cobertura', 'marca'];

  const rows = items.map((item) => [
    item.displayName,
    item.cityName,
    item.stateName ? `${item.stateCode} - ${item.stateName}` : item.stateCode,
    item.coverageLabel,
    item.brandLabel || '',
  ]);
  ...
  const link = document.createElement('a');
  link.href = url;
  link.download = 'rede_credenciada_driveb.csv';
  link.click();
  URL.revokeObjectURL(url);
}
...
export function buildMissingCitySummary(items) {
  const unresolvedItems = items.filter((item) => !item.hasResolvedCity);
  const byState = unresolvedItems.reduce((accumulator, item) => {
    const label = formatStateLabel(item.stateCode);
  ...
  return {
    ...
    sample: unresolvedItems.slice(0, 8),
  };
}
