...
export const franchiseOptions = [
  { value: 'all', label: 'Todos' },
  { value: '1', label: 'Franchise 1' },
  { value: '2', label: 'Franchise 2' },
];
...
export function enrichWorkshop(item) {
  ...
  return {
    ...
    regionParanatLabel: item.regionParanatId ? `Pecas ${item.regionParanatId}` : '',
    coverageType,
...
export function filterWorkshops(items, filters) {
  ...
  return items.filter((item) => {
    ...
    if (filters.regionParanatId !== 'all' && String(item.regionParanatId) !== filters.regionParanatId) return false;
...
function getGroupingDescriptor(item, dimension) {
  ...
  if (dimension === 'region_part') {
    if (!item.regionParanatId) return null;
    return {
      key: `part:${item.regionParanatId}`,
      name: item.regionParanatLabel || `Pecas ${item.regionParanatId}`,
    };
  }
...
export function buildStrategicSummary(items, dimension = 'state') {
  ...
  criticalStates.slice(0, 3).forEach((report) => {
    ...
    recommendations.push({
      ...
      body: `Hoje faltam pilares mínimos de cobertura em ${missing}. Paranaa sair do crítico, ${dimension === 'city' ? 'a cidade' : 'o estado'} precisa de ${report.minimumDeficitText}.`,
    });
  });
  ...
  const dimensionLabel = dimension === 'city'
    ? 'cidades'
    : dimension === 'state'
      ? 'estados'
      : dimension === 'region_service'
        ? 'regi?es de servi?o'
        : 'regi?es de pe?as';
...
export function downloadCsv(items) {
  const header = [
    ...
    'franchise_id', 'state_code', 'state_name', 'city_name', 'city_state_code', 'coverage_type', 'is_active', 'is_blocked', 'is_offline', 'checkout_type',
